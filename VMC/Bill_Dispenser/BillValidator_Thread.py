        ...
        while read:
            try:
                ...
                if str_data == '<':
                    ...
                    elif reading == 'B_BTY_13130000<' or reading == 'B_BTY_15150000<':
                        logging.info('Bills Enabled')

                    elif reading[:-2] == 'C_CDM_50':
...
class WriteThread(Thread):
    ...
    def write_cmd(self, cmd):
        ...
        try:
            if serial_port.isOpen():
                ...
                logging.info('Serial port NOT OPEN')
    ...
    def write_status(self):
        self.write_cmd(self.bill_commands.BILL_STATUS)
...
class BillValidator(Thread):
    ...
    vmc_commands = Commands.VmcCommands()
    number_of_coins = 100
    error_amount = 0
    ...
    def update_data(self):
        self.write_thread.write_cmd(self.vmc_commands.C_TUBE_STATUS)
        ...
        for d_id in xrange(1,5):
            json_data = json.dumps(data[d_id], default=lambda o: o.__dict__,
                                   sort_keys=True, indent=4)
            url = "http://localhost:8000/Currency/" + str(d_id) + "/"
            request = requests.put(url, data=json_data, headers=headers)

    def start_changer(self):
        self.write_thread.write_cmd(self.vmc_commands.C_SETUP)
        self.write_thread.write_cmd(self.vmc_commands.C_TUBE_STATUS)
        self.write_thread.write_cmd(self.vmc_commands.C_EXPANSION)
        self.write_thread.write_cmd(self.vmc_commands.disable_tubes())
        self.update_data()
    ...
    def write_dispense(self, units=0, cents=0):
            ...
            self.write_thread.write_cmd(self.vmc_commands.enable_tubes())
            # Check tubes status
            self.write_thread.write_cmd(self.vmc_commands.C_TUBE_STATUS)
            ...
            if quantity_10 != 0:
                ...
                self.write_thread.write_cmd(self.vmc_commands.check_hopper())
                self.write_thread.write_cmd(self.vmc_commands.hopper_dispense(quantity_10))
                self.number_of_coins -= quantity_10
            if quantity_5 != 0:
                print "Dispensing Changer coins = " + str(quantity_5)
                self.write_thread.write_cmd(self.vmc_commands.changer_dispense(4, quantity_5))
            if quantity_2 != 0:
                print "Dispensing Changer coins = " + str(quantity_2)
                self.write_thread.write_cmd(self.vmc_commands.changer_dispense(3, quantity_2))
            if quantity_1 != 0:
                print "Dispensing Changer coins = " + str(quantity_1)
                self.write_thread.write_cmd(self.vmc_commands.changer_dispense(2, quantity_1))
            if quantity_50c != 0:
                print "Dispensing Changer coins = " + str(quantity_50c)
                self.write_thread.write_cmd(self.vmc_commands.changer_dispense(0, quantity_50c))
    ...
    def socket_com(self, command=''):
        ...
        if cmd == 'ACCEPTBILL':
            ...
            return self.write_changer_accept()
